...
// Validate compliance using Azure OpenAI (OPTIMIZED - matches UI logic)
async function validateCompliance(applicationText, chapter, section, logger) {
  try {
    // Build detailed prompt with all elements for this chapter
...
CRITICAL: Return exactly ${chapter.elements.length} validation objects in the array, one for each requirement listed above.`;
    ...
    if (result.validations && Array.isArray(result.validations)) {
      ...
      logger.log(`Processed ${result.validations.length} validations: ${compliantItems.length} compliant, ${nonCompliantItems.length} non-compliant, ${notApplicableItems.length} N/A`);
    } else {
      logger.warning('No validations array found in response, attempting legacy format parsing');
      
      // Fallback to old format if new format not found
      if (result.compliantItems) compliantItems.push(...result.compliantItems);
      if (result.nonCompliantItems) nonCompliantItems.push(...result.nonCompliantItems);
      if (result.notApplicableItems) notApplicableItems.push(...result.notApplicableItems);
    }
